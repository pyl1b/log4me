...
from log4me.settings import LogSettings


def test_get_log_file(tmpdir, mocker):
    """Test the get_log_file function."""
    FAKE_TIME = datetime.datetime(2020, 12, 25, 17, 5, 55)
    mock_date = mocker.patch("log4me.setup_log.datetime")
    mock_date.now.return_value = FAKE_TIME

    from log4me.setup_log import get_log_file
    ...
    assert get_log_file(os.path.join(tmpdir, "log4me.log")).endswith(
        "log4me.log"
    )


def test_setup(tmpdir, mocker):
    from log4me.setup_log import setup_logging

    getLogger = mocker.patch("log4me.setup_log.logging.getLogger")
